...
export class Roulette extends Component<Props, State> {
  ...
  async componentWillReceiveProps(newProps) {
    await this.setState({
      arc: Math.PI / (newProps.options.length / 2)
    })
  ...
  drawRouletteWheel(props: Props = this.props) {
    const { options, baseSize } = props;
  ...
  spin() {
    this.spinTimer = null;
    this.setState({ spinTime: 0, spinTimeTotal: this.props.spinTimeTotal() }, () => this.rotate());
  }

  rotate(){
    const { spinAngleStart } = this.props;
    const { spinTimeTotal } = this.state;
  ...
  render() {
    const { baseSize, fullscreen } = this.props;

    return (
      <div className="roulette">
        <div className="roulette-container">
          <canvas ref={(c) => this.canvas = c} style={fullscreen ? 'width: 95vw' : ''} width={baseSize * 2} height={baseSize * 2} className="roulette-canvas"></canvas>
        </div>
